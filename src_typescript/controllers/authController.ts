...
class AuthController {
  ...
  public static async login(request: Request, response: Response) {
    ...
    requestParams.email = requestParams.email.toLowerCase().trim();
  ...
  public static async generatePasswordRecoveryKey(request: Request, response: Response) {
    ...
    //get email key given in path
    const path_email = request.params.email.toLowerCase().trim();
  ...
  public static async getUserIDFromToken(request: Request, response: Response) {
    ...
    if(!userPayload) {
        console.error("Controller Error: Missing JWT Payload");
        return response.status(500).json(toObj(response));
    }
